...
function shouldRetryToolCall(raw, body) {
  ...
  const actionRequest = /\b(use tools?|create|make|write|edit|read|inspect|summari[sz]e|understand|codebase|folder|file|bash|command)\b/i.test(user);
...
async function handleMessages(req, res, body) {
  ...
  let rawContent = (((upJson.choices || [])[0] || {}).message || {}).content || '';
  let parsed = parseToolCalls(rawContent, body.tools);
  ...
  if (hasTools && parsed.toolCalls.length === 0 && shouldRetryToolCall(rawContent, body)) {
