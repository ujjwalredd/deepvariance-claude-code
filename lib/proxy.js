...
const MAX_OUTPUT_TOKENS = parseInt(process.env.MAX_OUTPUT_TOKENS || '8192', 10); // output headroom ceiling
...
// Conservative token estimate (no tokenizer dependency). Real BPE tokenizers
// average ~3.5–4 chars/token on prose but far fewer on code/punctuation, so a
// flat chars/4 undershoots. We take the larger of chars/3.5 and a word+symbol
// count, biasing toward over-estimation — undershoot risks a hard context
// overflow, over-estimation only trims a little early (and the upstream
// overflow retry is the real safety net).
function approxTokens(str) {
  const s = str || '';
  if (!s) return 1;
  const byChars = Math.ceil(s.length / 3.5);
  const byTokens = (s.match(/\w+|[^\s\w]/g) || []).length; // words + individual symbols
  return Math.max(1, byChars, byTokens);
}
...
// Streaming variant: POST with stream:true and return the live Response so the
// caller can read res.body. Timeout guards the initial connect + first byte;
// once streaming, the caller drives reads. Errors here fall back to buffered.
async function postUpstreamStream(upstreamReq) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), UPSTREAM_TIMEOUT_MS);
  try {
    const up = await fetch(CHAT_URL, {
      method: 'POST',
      headers: upstreamHeaders(),
      body: JSON.stringify(Object.assign({}, upstreamReq, { stream: true })),
      signal: controller.signal,
    });
    if (!up.ok) {
      const text = await up.text();
      clearTimeout(timer);
      return { ok: false, status: up.status, text };
    }
    // Some backends ignore stream:true and answer with a normal JSON body.
    // Detect that and fall back to the buffered path rather than mis-reading it.
    const ctype = (up.headers.get('content-type') || '').toLowerCase();
    if (!ctype.includes('event-stream')) {
      clearTimeout(timer);
      return { ok: false, status: up.status, text: '', nonStream: true };
    }
    return { ok: true, status: up.status, body: up.body, cancel: () => { clearTimeout(timer); try { controller.abort(); } catch (_) {} }, done: () => clearTimeout(timer) };
  } catch (e) {
    clearTimeout(timer);
    const timedOut = e && (e.name === 'AbortError' || /abort/i.test(e.message || ''));
    return { ok: false, status: timedOut ? 504 : 502, text: '', networkError: e && e.message };
  }
}

// Parse an OpenAI SSE chunk stream into successive delta objects. Yields
// { content, toolCalls, done } for each `data:` line.
async function* iterateOpenAISSE(webStream) {
  ...
  while (true) {
    ...
    while ((nl = buf.indexOf('\n')) !== -1) {
      ...
      yield { content: delta.content || '', toolCalls: delta.tool_calls || null, raw: obj };
    }
  }
}
...
function emitMessageStart(res, msg) {
...
// Emit one complete content block (text or tool_use) as start/delta/stop.
function emitBlock(res, index, block) {
  if (block.type === 'text') {
    sse(res, 'content_block_start', { type: 'content_block_start', index, content_block: { type: 'text', text: '' } });
    if (block.text) sse(res, 'content_block_delta', { type: 'content_block_delta', index, delta: { type: 'text_delta', text: block.text } });
...
function emitMessageEnd(res, stopReason, outputTokens) {
  sse(res, 'message_delta', { type: 'message_delta', delta: { stop_reason: stopReason, stop_sequence: null }, usage: { output_tokens: outputTokens } });
  sse(res, 'message_stop', { type: 'message_stop' });
  res.end();
}

// Buffered (faux) stream: replay a fully-built Anthropic message as SSE. Used
// as the fallback when true streaming is unavailable.
function streamAnthropic(res, msg) {
  sseHead(res);
  emitMessageStart(res, msg);
  msg.content.forEach((block, i) => emitBlock(res, i, block));
  emitMessageEnd(res, msg.stop_reason, msg.usage.output_tokens);
}

// Tool-call markers whose *prefix* must be held back while streaming text, so
// a tag straddling chunk boundaries is never emitted as visible text.
const STREAM_MARKERS = ['<tool_call', '<tools', '<function_call', '```', '{"name"', '{ "name"'];

// Given accumulated (not-yet-emitted) text, return how many leading chars are
// safe to emit now: everything before the earliest complete marker, and never
// a trailing partial that could grow into a marker on the next chunk.
function safeEmitLength(buf) {
  let firstMarker = buf.length;
  for (const m of STREAM_MARKERS) {
    const idx = buf.indexOf(m);
    if (idx !== -1 && idx < firstMarker) firstMarker = idx;
  }
  if (firstMarker < buf.length) return firstMarker; // hold from the marker on
  // No complete marker: hold back any trailing suffix that is a prefix of one.
  let hold = 0;
  for (const m of STREAM_MARKERS) {
    const max = Math.min(m.length - 1, buf.length);
    for (let k = max; k > hold; k--) {
      if (buf.slice(buf.length - k) === m.slice(0, k)) { hold = k; break; }
    }
  }
  return buf.length - hold;
}

function earliestCompleteMarker(buf) {
  let best = -1;
  for (const m of STREAM_MARKERS) {
...
// True token-by-token streaming with a hold-back buffer so emulated tool-call
// tags never leak as visible text. Returns { handled } — handled:false means
// nothing was written to res and the caller should use the buffered path.
async function streamUpstream(res, body, ctx) {
  const nativeTools = ctx.wantsNativeTools;
  const baseMsgs = toOpenAIMessages(body, { nativeTools });
  const msgs = trimToBudget(baseMsgs, ctx.baseTargetIn);
  const inTok = totalTokens(msgs);
  const maxOut = Math.max(256, Math.min(ctx.reserveOut, MODEL_CTX - inTok - CTX_MARGIN));

  const upstreamReq = {
    model: UPSTREAM_MODEL,
    messages: msgs,
    max_tokens: maxOut,
    temperature: typeof body.temperature === 'number' ? (ctx.hasTools ? Math.min(body.temperature, 0.2) : body.temperature) : (ctx.hasTools ? 0.2 : 0.6),
  };
  if (typeof body.top_p === 'number') upstreamReq.top_p = body.top_p;
  if (Array.isArray(body.stop_sequences) && body.stop_sequences.length) upstreamReq.stop = body.stop_sequences;
  if (nativeTools) {
    ...
    if (body.tool_choice && body.tool_choice.disable_parallel_tool_use) upstreamReq.parallel_tool_calls = false;
  }

  const up = await postUpstreamStream(upstreamReq);
  if (!up.ok || !up.body) {
    // Let auto-mode learn native tools are unsupported so the buffered
    // fallback skips straight to emulated instead of retrying native.
    if (nativeTools && ctx.allowEmulatedFallback && TOOL_MODE === 'auto' && upstreamSupportsFallback(up.status, up.text)) {
      nativeToolsKnownUnsupported = true;
    }
    return { handled: false };
  }

  // We have a live stream: from here on we own the response.
  const msg = {
    id: rid('msg_'), type: 'message', role: 'assistant',
    model: body.model || UPSTREAM_MODEL,
    usage: { input_tokens: inTok, output_tokens: 0, cache_read_input_tokens: 0, cache_creation_input_tokens: 0 },
  };
  sseHead(res);
  emitMessageStart(res, msg);

  let raw = '', pending = '', buffered = '';
  let idx = 0, textOpen = false, buffering = false, sawTool = false;
  const nativeAccum = {};
  ...
  const closeText = () => {
    if (textOpen) { sse(res, 'content_block_stop', { type: 'content_block_stop', index: idx }); idx++; textOpen = false; }
  };
  const emitText = (s) => { if (!s) return; ensureText(); sse(res, 'content_block_delta', { type: 'content_block_delta', index: idx, delta: { type: 'text_delta', text: s } }); };

  try {
    for await (const chunk of iterateOpenAISSE(up.body)) {
      if (chunk.done) break;
      if (nativeTools && chunk.toolCalls) {
        for (const d of chunk.toolCalls) {
          const i = d.index || 0;
          const cur = nativeAccum[i] || (nativeAccum[i] = { id: '', name: '', args: '' });
          if (d.id) cur.id = d.id;
          if (d.function) {
      ...
      if (!c) continue;
      raw += c;
      if (buffering) { buffered += c; continue; } // in the tool region
      if (nativeTools) { emitText(c); continue; } // native: no tag hold-back
      pending += c;
      const markerAt = earliestCompleteMarker(pending);
      if (markerAt !== -1) {
        emitText(pending.slice(0, markerAt));
        closeText();
        // Everything from the marker on is the tool region; parse only this so
        // the already-streamed prose is never re-emitted as a residual block.
        buffered += pending.slice(markerAt);
        buffering = true; sawTool = true; pending = '';
      } else {
        const n = safeEmitLength(pending);
        if (n > 0) { emitText(pending.slice(0, n)); pending = pending.slice(n); }
      }
    }
    if (up.done) up.done();
  } catch (e) {
    log('stream read error', e && e.message);
    // Bytes already sent — close the stream cleanly rather than error out.
    closeText();
    emitMessageEnd(res, 'end_turn', approxTokens(raw));
    ctx.summary(200, { native: nativeTools, stream_error: e && e.message, out_tok: approxTokens(raw) });
    return { handled: true };
  }
  ...
  let residual = '';
  if (nativeTools) {
    for (const k of Object.keys(nativeAccum)) {
      const a = nativeAccum[k];
      const call = toAllowedCall({ name: a.name, arguments: a.args }, new Set((body.tools || []).map(t => t && t.name).filter(Boolean)));
      if (call) { call.id = a.id || call.id; toolCalls.push(call); }
    }
  } else if (buffering) {
    // Parse only the buffered tool region; residual is text that followed the
    // tool call (rare), never the prose we already streamed.
    const parsed = parseToolCalls(buffered, body.tools);
    toolCalls = parsed.toolCalls;
    residual = parsed.text;
  } else {
    // Never entered a tool region: flush the held-back tail as text.
    emitText(pending); pending = '';
  }

  closeText();
  for (const tc of toolCalls) { emitBlock(res, idx, { type: 'tool_use', id: tc.id, name: tc.name, input: tc.input }); idx++; }
  if (residual && residual.trim()) { emitBlock(res, idx, { type: 'text', text: residual }); idx++; }

  const outTok = approxTokens(raw);
  emitMessageEnd(res, toolCalls.length ? 'tool_use' : 'end_turn', outTok);
  ctx.summary(200, { native: nativeTools, streamed: true, in_tok: inTok, out_tok: outTok, tool_calls: toolCalls.length });
  return { handled: true };
}
...
async function handleMessages(req, res, body) {
  ...
  // Capped by MAX_OUTPUT_TOKENS (default 8192) rather than a hard 4096.
  const reserveOut = Math.min(Math.max(body.max_tokens || MAX_OUTPUT_TOKENS, 512), MAX_OUTPUT_TOKENS);

  // Retry loop: trim input to a target, send; if the backend still reports a
  // context-length overflow (our estimate can undershoot the real
  ...
  // True streaming path: stream tokens as they arrive. Falls back to the
  // buffered path below if the backend can't stream (nothing written yet).
  if (wantStream) {
    try {
      const streamed = await streamUpstream(res, body, {
        hasTools, wantsNativeTools, allowEmulatedFallback, reserveOut, baseTargetIn, summary,
      });
      if (streamed.handled) return;
      log('streaming unavailable -> buffered faux-stream fallback');
    } catch (e) {
      log('streamUpstream threw before output', e && e.message);
